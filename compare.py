...
class LogoInfringementDetector:
    ...
    def extract_vit_features(self, image_path):
        """Extract deep features from the logo using Vision Transformer"""
        ...
        # Extract features
        with torch.no_grad():
            # ViT returns the [CLS] token which contains the global image representation
    ...
    def calculate_visual_similarity(self, image_path1, image_path2):
        """Calculate visual similarity using SSIM and ViT features"""
        # Calculate SSIM
        ...
        # Calculate ViT feature similarity
        features1 = self.extract_vit_features(image_path1)
        features2 = self.extract_vit_features(image_path2)
        
        # Calculate cosine similarity between feature vectors
        vit_sim = 1 - cosine(features1, features2)
        
        # Combine SSIM and ViT feature similarity
        visual_sim = (ssim_score + vit_sim) / 2 * 100
        
        return visual_sim, ssim_score * 100, vit_sim * 100
    ...
    def detect_infringement(self, logo1_path, logo2_path, threshold=55.0):
        ...
        with st.spinner('Analyzing visual and color similarity...'):
            ...
            visual_similarity, ssim_similarity, vit_similarity = self.calculate_visual_similarity(logo1_path, logo2_path)
            
            results['color_similarity'] = color_similarity
            results['visual_similarity'] = visual_similarity
            results['ssim_similarity'] = ssim_similarity
            results['vit_similarity'] = vit_similarity
            
            # Calculate weighted final similarity score
            # Give more weight to ViT similarity as it captures semantic features better
            final_similarity = (0.3 * text_similarity + 
            ...
            # Determine if infringement is detected
            if final_similarity >= threshold:
                results['infringement_detected'] = True
                
                # Determine the type of infringement
                if text_similarity >= threshold and vit_similarity >= threshold:
                    results['infringement_type'] = 'Text and Visual Similarity'
                elif text_similarity >= threshold:
                    results['infringement_type'] = 'Text Similarity'
                else:
                    results['infringement_type'] = 'Visual Similarity'
            else:
                results['infringement_detected'] = False
                results['infringement_type'] = 'None'
...
def main():
    ...
    - Visual similarity using Vision Transformer (ViT)
    - Color similarity through dominant color extraction
    ...
    if logo1_file and logo2_file:
        ...
        if st.button("Analyze Logos for Infringement"):
            with st.spinner("Analyzing logos for potential infringement..."):
                ...
                with col2:
                    st.metric("Visual Similarity", f"{results.get('visual_similarity', 0):.1f}%")
                with col3:
                    st.metric("Final Similarity", f"{results.get('final_similarity', 0):.1f}%")
                ...
                similarity_data = {
                    'Metric': ['Text Similarity', 'Color Similarity', 'SSIM Similarity', 'ViT Similarity', 'Final Similarity'],
                    'Score (%)': [
                        f"{results.get('text_similarity', 0):.1f}%",
                        f"{results.get('color_similarity', 0):.1f}%",
                        f"{results.get('ssim_similarity', 0):.1f}%",
                        f"{results.get('vit_similarity', 0):.1f}%",
                        f"{results.get('final_similarity', 0):.1f}%"
                    ],
                    'Weight': ['30%', '20%', '10%', '40%', '100%']
                }
                ...
                chart_data = pd.DataFrame({
                    'Metric': ['Text', 'Color', 'SSIM', 'ViT', 'Final'],
                    'Score': [
                        results.get('text_similarity', 0),
                        results.get('color_similarity', 0),
                        results.get('ssim_similarity', 0),
                        results.get('vit_similarity', 0),
                        results.get('final_similarity', 0)
                    ]
                })
                ...
                with st.expander("Detailed Analysis Results"):
                    st.json({
                        ...
                        "ssim_similarity": f"{results.get('ssim_similarity', 0):.2f}%",
                        "vit_similarity": f"{results.get('vit_similarity', 0):.2f}%",
